...
// import 'package:nemuru/screens/ai_response_screen.dart'; // 星空背景と流れ星のペインターをインポート (一時的にコメントアウト)
import 'package:nemuru/widgets/star_sky_background.dart'; // Starクラスのインポート
...
class _CheckInScreenState extends State<CheckInScreen> with SingleTickerProviderStateMixin {
  ...
  final Map<String, int> _moodIconIds = {
    for (int i = 0; i < AppConstants.moodTypes.length; i++)
      AppConstants.moodTypes[i]: AppConstants.moodIconStartId + i,
  };
  ...
  // 流れ星アニメーション用のコントローラー (一時的にコメントアウト)
  ...
  void initState() {
    ...
    // 流れ星アニメーションの設定 (一時的にコメントアウト)
    // _shootingStarController = AnimationController(
    //   vsync: this,
    //   duration: UIConstants.shootingStarAnimationDuration,
    // );
  ...
  void _generateStars() {
    final random = Random();
    _stars = List.generate(UIConstants.darkModeStarCount, (index) {
      return Star(
        x: random.nextDouble(),
        y: random.nextDouble(),
  ...
  void dispose() {
    ...
    // _shootingStarController.dispose(); // アニメーションコントローラーのクリーンアップ (一時的にコメントアウト)
    super.dispose();
  }
  ...
  Future<void> _submitCheckIn() async {
    ...
    if (_selectedMood == null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(AppConstants.moodNotSelectedError),
    ...
    // サービスを取得してキャッシュ（非同期処理間での状態変更を防ぐ）
    ...
    final hasReachedLimit = isPremium 
        ? subscriptionService.hasReachedPremiumLimit 
        : subscriptionService.hasReachedFreeLimit;
                ...
                child: CustomPaint(
                  painter: StarSkyPainter(
                    ...
                    stars: Theme.of(context).brightness == Brightness.dark
                        ...
                        : _stars.take(UIConstants.lightModeStarCount).toList(), // ライトモードでは定数で指定した数のみ
                  ),
                ),
              ),
              // 流れ星アニメーション (一時的にコメントアウト)
              // Positioned.fill(
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: UIConstants.moodGridCrossAxisCount,
          childAspectRatio: UIConstants.moodGridChildAspectRatio,
