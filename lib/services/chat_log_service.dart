...
class ChatLogService extends ChangeNotifier {
  ...
  // ローカル保存用のキー（定数から取得）
  static const String _logsKey = AppConstants.chatLogsKey;
    ...
    try {
      ...
      for (final logJson in logsJson) {
        try {
          ...
          // Skip invalid log entries but log the error in debug mode
          if (kDebugMode) {
            print('Failed to parse chat log: $e');
          }
        }
      }
      ...
      // Log critical loading errors
      if (kDebugMode) {
        print('Critical error loading chat logs: $e');
      }
      // Consider notifying listeners even on error to prevent UI hanging
      notifyListeners();
    }
  }
  ...
  Future<void> _saveLogsToLocal() async {
    try {
      ...
      if (kDebugMode) {
        print('Failed to save chat logs: $e');
      }
      rethrow; // Don't silently fail on save operations
    }
  }
  ...
  Future<ChatLog> createLog({
    ...
    final sanitizedMood = mood.trim();
    if (sanitizedMood.isEmpty) {
      throw ArgumentError('Mood cannot be empty');
    }
    if (!AppConstants.moodTypes.contains(sanitizedMood)) {
      throw ArgumentError('Invalid mood type: $sanitizedMood');
    }
    if (characterId < 0 || characterId >= AppConstants.totalCharacterCount) {
      throw ArgumentError('Invalid character ID: $characterId');
    }
    
    String? sanitizedReflection;
    if (reflection != null) {
      sanitizedReflection = reflection.trim();
      if (sanitizedReflection.length > AppConstants.maxUserInputLength) {
        throw ArgumentError('Reflection text too long');
      }
      // Remove potentially harmful characters
      sanitizedReflection = sanitizedReflection.replaceAll(RegExp(r'[^\w\s\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF.,!?()「」]'), '');
    }
    ...
    final limit = isPremium 
        ? AppConstants.premiumConversationLimit 
        : AppConstants.freeConversationLimit;
        
    // 既に制限に達している場合はエラーをスロー
    if (todayCount >= limit) {
      throw Exception(AppConstants.conversationLimitReachedMessage);
    }
  ...
  Future<void> deleteLog(String id) async {
    try {
      ...
      if (kDebugMode) {
        print('Failed to delete chat log: $e');
      }
      rethrow; // Don't silently fail on delete operations
    }
  }
  ...
  Future<void> updateLogSummary(String logId, String summary, {List<Message>? fullConversation}) async {
    ...
    if (logId.trim().isEmpty) {
      throw ArgumentError('Log ID cannot be empty');
    }
    if (summary.trim().isEmpty) {
      throw ArgumentError('Summary cannot be empty');
    }
    if (summary.length > AppConstants.maxSystemPromptLength) {
      throw ArgumentError('Summary too long');
    }
  ...
  List<String> _extractCommonTopics() {
    ...
    for (final log in _logs) {
      final texts = <String>[];
      if (log.reflection?.isNotEmpty == true) texts.add(log.reflection!);
      if (log.summary?.isNotEmpty == true) texts.add(log.summary!);
