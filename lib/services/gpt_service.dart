...
class GPTService {
  // バックエンドAPIのエンドポイント（定数から取得）
  static const String _chatCompletionsBaseUrl = ApiConstants.chatCompletionEndpoint;
  static const String _summarizeBaseUrl = ApiConstants.summarizeEndpoint;
  ...
  // 会話履歴を適切なサイズに制限するメソッド
  void _trimConversationHistory() {
    // 最大履歴数の2倍を超えた場合、古いメッセージを削除
    const maxHistorySize = ApiConstants.maxConversationHistory * 2;
    if (_conversationHistory.length > maxHistorySize) {
      final messagesToRemove = _conversationHistory.length - ApiConstants.maxConversationHistory;
  ...
  void addUserMessage(String userInput) {
    ...
    if (userInput.trim().isEmpty) {
      throw ArgumentError('ユーザー入力が空です');
    }
    
    // 文字数制限の確認（AppConstantsから取得）
    if (userInput.length > AppConstants.maxUserInputLength) {
      throw ArgumentError('入力文字数が制限を超えています（最大${AppConstants.maxUserInputLength}文字）');
    }
    
    final userMessage = Message(content: userInput.trim(), isUser: true);
    _conversationHistory.add(userMessage);
    _messageCount++; // ユーザーメッセージは常にカウント
    
    // メモリ使用量を制限するため、履歴が長くなりすぎた場合は古いメッセージを削除
  ...
  Future<String> generateAndAddAIResponse({String? initialContextOverride, BuildContext? context}) async {
    ...
    try {
      ...
      // デバッグ用ログ出力（詳細なエラー情報を記録）
      debugPrint('GPTService.generateAndAddAIResponse エラー: $e');
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'max_tokens': ApiConstants.conversationMaxTokens, 
          'temperature': ApiConstants.conversationTemperature,
        }),
      ).timeout(ApiConstants.apiTimeout, onTimeout: () {
        // タイムアウトの場合
        throw TimeoutException(ApiConstants.timeoutErrorMessage);
      });
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (data['choices'] != null && 
            ...
            throw Exception('APIレスポンスの形式が不正です。');
          }
        } catch (e) {
      ...
      // デバッグ用ログ出力（詳細なエラー情報を記録）
      debugPrint('GPTService._generateResponseFromAPI エラー: $e');
...
''';
  ...
  int _getSelectedCharacterId() {
    try {
      ...
      // デバッグ用ログ出力（詳細なエラー情報を記録）
      debugPrint('GPTService._getSelectedCharacterId エラー: $e');
      return 0;
    }
  }
...
「疲れを感じていることを話し、休息の大切さについて話し合った」''',
    ...
    try {
      final response = await http.post(
        Uri.parse(_summarizeBaseUrl),
        headers: ApiConstants.authHeaders,
        body: jsonEncode({
          'model': ApiConstants.gptModel,
          'messages': messagesForSummary,
          'max_tokens': ApiConstants.summaryMaxTokens, 
          'temperature': ApiConstants.summaryTemperature,
        }),
      );
      ...
      // デバッグ用ログ出力（詳細なエラー情報を記録）
      debugPrint('GPTService.summarizeConversation エラー: $e');
